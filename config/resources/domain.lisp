...
(define-resource process ()
  ...
  :has-one (
    (group :via ,(s-prefix "dct:publisher") :as "publisher")
    (processStatistic :via ,(s-prefix "ext:hasStatistics") :as "process-statistics")
    (conceptualProcess :via ,(s-prefix "dct:source") :as "linked-concept")
  )
  :has-many (
    (file :via ,(s-prefix "nie:isPartOf") :inverse t :as "files")
    (ipdcProduct :via ,(s-prefix "cpsv:follows") :inverse t :as "ipdc-products")
    (administrative-unit-classification-codes
      :via ,(s-prefix "icr:isRelevantForAdministrativeUnit")
      :as "relevant-administrative-units")
    (informationAsset
      :via ,(s-prefix "icr:hasInformationAsset")
      :as "information-assets")
    (process :via ,(s-prefix "prov:wasInfluencedBy") :as "linked-blueprints")
    (group :via ,(s-prefix "prov:usedBy") :as "users")
    (link :via ,(s-prefix "rdfs:seeAlso") :as "links")
  )
